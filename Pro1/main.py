...
def get_car_by_id(request:Request,id: int = Path(...,ge=0,lt=1000)):
    car = cars.get(id)
    if not car:
        templates.TemplateResponse("search.html", {"request":request,"car":car,"id":id},status_code=status.HTTP_404_NOT_FOUND )
    return templates.TemplateResponse("search.html", {"request":request,"car":car,"id":id},status_code=status.HTTP_200_OK )
